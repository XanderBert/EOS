...
function(eos_require_fastgltf TARGET_NAME)
    eos_set_option(FASTGLTF_ENABLE_TESTS OFF BOOL)
    eos_set_option(FASTGLTF_ENABLE_EXAMPLES OFF BOOL)
    eos_set_option(FASTGLTF_ENABLE_DOCS OFF BOOL)
    eos_set_option(FASTGLTF_ENABLE_GLTF_RS OFF BOOL)
    eos_set_option(FASTGLTF_ENABLE_ASSIMP OFF BOOL)
    eos_set_option(FASTGLTF_ENABLE_CPP_MODULES OFF BOOL)
    eos_set_option(FASTGLTF_DOWNLOAD_SIMDJSON ON BOOL)

    if(NOT TARGET fastgltf::fastgltf)
        eos_dep(fastgltf GIT https://github.com/spnda/fastgltf TAG ${EOS_DEP_FASTGLTF_TAG})
        eos_dep_add_subdirectory(fastgltf BINARY_DIR "${CMAKE_BINARY_DIR}/fastgltf-build" EXCLUDE_FROM_ALL)
    endif()
...
function(eos_require_ktx TARGET_NAME)
    ...
    eos_set_option(KTX_FEATURE_TOOLS_CTS OFF BOOL)
    eos_set_option(KTX_FEATURE_DOC OFF BOOL)
    eos_set_option(KTX_FEATURE_JNI OFF BOOL)
    eos_set_option(KTX_FEATURE_PY OFF BOOL)
    eos_set_option(KTX_FEATURE_LOADTEST_APPS OFF STRING)
    eos_set_option(BUILD_SHARED_LIBS OFF BOOL)
    eos_set_option(KTX_FEATURE_KTX1 ON BOOL)
    eos_set_option(KTX_FEATURE_KTX2 ON BOOL)
    ...
    if(NOT TARGET ktx AND NOT TARGET ktx_read)
        eos_dep(fetch_ktx GIT https://github.com/KhronosGroup/KTX-Software TAG ${EOS_DEP_KTX_TAG})
        eos_dep_add_subdirectory(fetch_ktx BINARY_DIR "${CMAKE_BINARY_DIR}/fetch_ktx-build" EXCLUDE_FROM_ALL)
    endif()

    # Link read-only KTX when texture tooling is disabled to avoid encoder code.
    if(EOS_BUILD_TEXTURE_TOOLS)
        target_link_libraries(${TARGET_NAME} PUBLIC ktx)
    else()
        target_link_libraries(${TARGET_NAME} PUBLIC ktx_read)
    endif()
endfunction()
